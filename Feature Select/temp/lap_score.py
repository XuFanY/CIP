...
# path{single,multi},score{fisher,laplace}
def fisher_laplace(F, Lbls, k, filename='xxx', path=sglDst, score='fisher'):
    ...
    for i in range(k):
        X.T[i] = F.T[index[i]]
    with open(path + '%s-%s-%s.arff' % (score, filename, k), 'w') as wf:
        wf.write(header(k))
        wf.write(mStr(np.c_[X.astype(np.str), Lbls]))
        wf.close()
    print(np.transpose(flscore))
    return np.transpose(flscore)


def lap_score(F, Lbls, k, filename='xxx', path=sglDst):
    """
    ...
    # the smaller the laplacian score is, the more important the feature is
    index = np.argsort(lap_score)[:k]
    print(index)
    n = np.shape(F)[0]
    X = np.zeros((n, k))
    for i in range(k):
        X.T[i] = F.T[index[i]]
    with open(path + 'laplace-%s-%s.arff' % (filename, k), 'w') as wf:
        wf.write(header(k))
        wf.write(mStr(np.c_[X.astype(np.str), Lbls]))
        wf.close()

    print(np.transpose(lap_score))
